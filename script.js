...
app.get('/', (req, res) => {
    res.send('Welcome to Shuddhas REST API');
});
...
//If there is no valid customer ID, then display an error with the following message
    if (!customers) res.status(404).send('<h2 style="font-family:Malgun Gothic; color: darkred;">Oops...Cant find what you are looking for!</h2>');
        res.send(customer);
});
